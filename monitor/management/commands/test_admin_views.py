    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not user:
                ...
                self.stdout.write(f"Created test superuser: testadmin / testpass123")
            else:
        ...
        try:
            ...
            if response.status_code != 200:
                self.stdout.write(self.style.ERROR(f"Admin index error: {response.status_code}"))
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 500:
                self.stdout.write(self.style.ERROR(f"STILL BROKEN: RecordLog admin returns 500!"))
                ...
                self.stdout.write(self.style.SUCCESS("SUCCESS: RecordLog admin works now!"))
            else:
                self.stdout.write(self.style.WARNING(f"RecordLog list: HTTP {response.status_code}"))
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 500:
                self.stdout.write(self.style.ERROR(f"RecordLogIPInfo admin returns 500!"))
            elif response.status_code == 200:
            ...
            self.stdout.write(self.style.ERROR(f"Error accessing RecordLogIPInfo admin: {e}"))

        # Test other admin views to make sure we didn't break anything
        models_to_test = ['domain', 'ipwhoisinfo', 'domainsnapshot', 'apikey', 'monitorsettings']
        for model in models_to_test:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.stdout.write(self.style.WARNING(f"{model} admin: HTTP {response.status_code}"))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Error accessing {model} admin: {e}"))
