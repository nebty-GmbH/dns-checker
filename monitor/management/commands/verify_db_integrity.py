    ...
    def handle(self, *args, **options):
        ...
        for ip_info in RecordLogIPInfo.objects.all():
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Error checking record_log for RecordLogIPInfo {ip_info.id}: {e}"
                    )
                )
                broken_record_refs.append(ip_info.id)
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Error checking ip_whois_info for RecordLogIPInfo {ip_info.id}: {e}"
                    )
                )
                broken_whois_refs.append(ip_info.id)
        ...
        if broken_record_refs:
            self.stdout.write(
                self.style.WARNING(
                    f"Found {len(broken_record_refs)} RecordLogIPInfo entries with broken record_log references: {broken_record_refs}"
                )
            )
        ...
        if broken_whois_refs:
            self.stdout.write(
                self.style.WARNING(
                    f"Found {len(broken_whois_refs)} RecordLogIPInfo entries with broken ip_whois_info references: {broken_whois_refs}"
                )
            )
        ...
        for record_log in RecordLog.objects.all()[
            :100
        ]:  # Check first 100 to avoid overwhelming
            try:
                # Test the relationships that admin methods use
                _ = record_log.domain.name

                # Test ip_info_entries relation
                _ = record_log.ip_info_entries.count()

                # Test snapshot relation
                has_snapshot = hasattr(record_log, "snapshot")
                if has_snapshot:
                    try:
                        _ = record_log.snapshot
                    except Exception:
                        pass

            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"Error testing RecordLog {record_log.id}: {e}")
                )
        ...
        if problematic_record_logs:
            self.stdout.write(
                self.style.WARNING(
                    f"Found {len(problematic_record_logs)} problematic RecordLog entries: {problematic_record_logs}"
                )
            )
        ...
        if options["fix"]:
            ...
            if broken_record_refs or broken_whois_refs:
                ...
                deleted_count = RecordLogIPInfo.objects.filter(
                    id__in=all_broken
                ).delete()[0]
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Deleted {deleted_count} broken RecordLogIPInfo entries"
                    )
                )
