...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--domain',
            type=str,
            help='Check a specific domain by name'
        )
        parser.add_argument(
            '--all',
            action='store_true',
        ...
        if domain_name:
            ...
            for domain_result in result['results']:
                if domain_result['success']:
                    status = 'CHANGED' if domain_result.get('is_change', False) else 'NO CHANGE'
                    ips = ', '.join(domain_result.get('ips', []))
                    self.stdout.write(
                        self.style.SUCCESS(f"  ✓ {domain_result['domain']}: {ips} ({status})")
                    )
                else:
                    self.stdout.write(
                        self.style.ERROR(f"  ✗ {domain_result['domain']}: {domain_result['error']}")
                    )
            ...
            self.stdout.write(f"Scheduling checks for {active_domains.count()} active domains...")
            ...
            for domain in active_domains:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"  Failed to schedule {domain.name}: {str(e)}")
                    )
